...
public:
    ...
    Enemy(glm::vec3 startPos, int team) : Entity("Enemy") {
        ...
        shadow->AddComponent<MeshRenderer>("Plane", glm::vec3(0.0f, 0.0f, 0.0f)); // 暺
        // 蝔凝瘚桀?唳銝?暺?嚗??Z-Fighting
        shadow->transform->position = transform->position + glm::vec3(0, 0.02f, 0);
        shadow->transform->scale = glm::vec3(1.2f, 1.0f, 1.2f); // ?啣蔣瘥犖蝔凝憭找?暺?
