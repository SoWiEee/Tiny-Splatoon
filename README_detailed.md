...
Tiny-Splatoon 是一個用 **C++17 + OpenGL** 製作的 3D 小型多人連線遊戲。玩家分成紅/綠兩隊，在地圖上移動與射擊，並透過「塗地（ink）」機制影響比分與移動行為。多人連線使用 **Valve GameNetworkingSockets**，採用 **listen server**（Host 同時是玩家）模型：一台玩家電腦同時扮演 Server + 本機玩家，其他玩家以 Client 連線加入（最多 8 人房：Host + 7 clients）。
...
此順序的意義：  
- 若你把「收包」放在 Update 之後，會造成畫面/狀態落後一幀，UI 尤其明顯。  
- 若你把「UI 更新」做成靠網路封包觸發，則 OnPacket 需要可靠、且 header.type 必須正確（此專案的封包 type 是靠 PacketHeader 解析出來）。
...
職責：計算 delta time（dt）。  
互動：
- main.cpp：`timer.Tick()` → `timer.GetDeltaTime()`。
...
注意：本專案的 `ReceivedPacket.type` 是從封包第一個 byte 的 PacketHeader.type 解析，因此 server 回送 accept 時必須填 `pkt.header.type = S2C_JOIN_ACCEPT`。
