...
void GUIManager::DrawLobby(bool& outStartGame) {
    ...
    if (ImGui::Begin("LobbyOverlay", nullptr, flags)) {
        ...
        // �p�G���ܧ�A�o�Ө禡�|�^�� true�A�ç�s currentWeapon
        ...
        // 3. ���s (Server ��� Start, Client ��� Waiting)
...
void GUIManager::DrawLobbyCircles(int w, int h) {
    ...
    for (int i = 0; i < kLobbySlotCount; i++) {
        ...
        if (lobbySlots[i].playerID != -1) {
            if (lobbySlots[i].teamID == 1) color = IM_COL32(255, 50, 50, 255); // ��
            if (lobbySlots[i].teamID == 2) color = IM_COL32(50, 255, 50, 255); // ��
        }
        ...
        // �� Cursor ������U��
        ImGui::SetCursorPos(ImVec2(x - nameW * 0.5f - 10, y + radius + 15)); // �L�զ�m
...
bool GUIManager::DrawWeaponSelector(WeaponType& currentSelection) {
    ...
    auto DrawWeaponBtn = [&](const char* label, WeaponType type) {
        ...
        if (currentSelection == type) {
            ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.2f, 0.8f, 0.2f, 1.0f)); // �G���
        }
        else {
            ImGui::PushStyleColor(ImGuiCol_Button, ImVec4(0.3f, 0.3f, 0.3f, 1.0f)); // �Ǧ�
        }
